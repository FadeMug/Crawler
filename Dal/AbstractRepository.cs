...
namespace Dal
{
        ...
        where T : class
    {
        ...
        public virtual List<T> SqlQuery(string sql)
        {
            return _entities.Database.SqlQuery<T>(sql).ToList();
        }

        public virtual PageData<T> SqlQuery(string sql, int pageIndex, int pageSize, bool groupby = false, string orderbyStr = "")
        {
            long count = 0;
            pageIndex = pageIndex <= 0 ? 0 : pageIndex;

            StringBuilder sb = new StringBuilder();
            sb.Append(@"WITH _TEMP AS(");
            sb.Append(sql);
            sb.Append(")");
            sb.AppendFormat(" SELECT * FROM _TEMP where _rowNum between {0}  and {1} ", ((pageIndex - 1) * pageSize) + 1, pageIndex * pageSize);
            const string queryCount = " SELECT COUNT(*) AS _totalCount FROM ({0})a ";

            var list = _entities.Database.SqlQuery<T>(sb.ToString()).ToList();
            //查行数
            count = _entities.Database.SqlQuery<int>(string.Format(queryCount, sql)).SingleOrDefault();

            return new PageData<T>(count, pageIndex, pageSize, list);
        }
